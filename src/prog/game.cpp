...
void Game::processCode(Com::Code code, const uint8* data) {
	...
	case Com::Code::pieces:
		for (uint16 i = 0; i < config.numPieces; i++)
			pieces.ene(i)->setPos(gtop(idToPos(reinterpret_cast<const uint16*>(data)[i])));
		...
		break;
	case Com::Code::move:
		pieces.ene(reinterpret_cast<const uint16*>(data)[0])->setPos(gtop(idToPos(reinterpret_cast<const uint16*>(data)[1])));
		break;
	case Com::Code::kill:
...
void Game::setBgrid() {
	...
	for (int16 x = 0; x <= config.homeWidth; x++)
		for (int16 y = -int16(config.homeHeight); y <= config.homeHeight + 1; y += boardHeight)
			GMesh::setVertex(verts.data(), i++, gtop(vec2s(x, y), -0.018f) + vec3(-(config.objectSize / 2.f), 0.f, -(config.objectSize / 2.f)), Camera::up, vec2(0.f));
...
void Game::sendSetup() {
	...
	for (uint16 i = 0; i < config.numPieces; i++)
		reinterpret_cast<uint16*>(netcp->sendb + 1)[i] = invertId(posToId(ptog(pieces.own(i)->getPos())));
	netcp->sendData();
}
