...
const string& enumToStr(const array<string, N>& names, T id) {
	return sizet(id) < names.size() ? names[sizet(id)] : emptyStr;
}
...
template <class T>
const string& valToStr(const umap<T, string>& names, uint8 jhat) {
	try {
		return names.at(jhat);
